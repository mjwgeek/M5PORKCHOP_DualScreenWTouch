...
## Touch wiring note (v4)
This build keeps the original dedicated touch wiring: T_CLK=GPIO15, T_CS=GPIO4, T_DIN=GPIO13, T_DO=GPIO8, T_IRQ=GPIO9. GPIO8/GPIO9 conflict with the Cardputer keyboard while connected, including in M5Launcher. Disconnect those touch wires when using M5Launcher. The external touch menu redraw is event-driven to prevent full-screen flashing.
