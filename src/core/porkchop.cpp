...
void Porkchop::update() {
    ...
    // External touch is intentionally limited to the full-screen root menu.
    // The former top/bottom margin navigation was unreliable across screens
    // and has been removed; do not generate invisible navigation events.
    handleInput();
    yield(); // Allow other tasks to run between operations
...
void Porkchop::handleInput() {
    ...
    // A fresh touch on the external screen while idle opens the same root
    // menu as the Cardputer ` / ~ (ESC) key. Use the hardware IRQ only;
    // coordinate data is not needed until the menu is already visible.
    // Latch the press until release so the opening touch cannot also select
    // the button that appears under the user's finger.
    static bool idleTouchWasDown = false;
    const bool idleTouchDown = extTouchPressed();
    if (!idleTouchDown) {
        idleTouchWasDown = false;
    } else if (currentMode == PorkchopMode::IDLE && !idleTouchWasDown) {
        idleTouchWasDown = true;
        Display::resetDimTimer();
        setMode(PorkchopMode::MENU);
        return;
    }
    
    const bool keyboardChanged = M5Cardputer.Keyboard.isChange();
    const bool touchNavPending = extTouchNavPending();
    if (!keyboardChanged && !touchNavPending) return;
    
    // Any keyboard or external navigation input resets the screen dim timer.
    Display::resetDimTimer();
    ...
    if (currentMode == PorkchopMode::PIGSYNC_DEVICE_SELECT) {
        ...
        if (deviceCount > 0) {
            if (M5Cardputer.Keyboard.isKeyPressed(';') || extTouchConsume(ExtTouchNav::UP)) {
            ...
            if (M5Cardputer.Keyboard.isKeyPressed('.') || extTouchConsume(ExtTouchNav::DOWN)) {
        ...
        if ((M5Cardputer.Keyboard.isKeyPressed(KEY_ENTER) || extTouchConsume(ExtTouchNav::SELECT)) && PigSyncMode::getDeviceCount() > 0) {
        ...
        if (extTouchConsume(ExtTouchNav::BACK)) {
            PigSyncMode::stopDiscovery();
            PigSyncMode::stop();
            setMode(PorkchopMode::MENU);
            return;
    ...
    if (M5Cardputer.Keyboard.isKeyPressed(KEY_ENTER) || extTouchConsume(ExtTouchNav::SELECT)) {
        if (currentMode == PorkchopMode::ABOUT) {
            Display::onAboutEnterPressed();
            return;
        }
    }
    if (currentMode == PorkchopMode::ABOUT && extTouchConsume(ExtTouchNav::BACK)) {
        setMode(PorkchopMode::MENU);
        return;
    }
