...
void Display::renderLivingBackdrop() {
    // The root/group menu becomes a large touch UI on the external panel. Other
    // text screens keep the animated backdrop and remain keyboard-controlled.
    if (porkchop.getMode() == PorkchopMode::MENU && Menu::isActive()) {
        Menu::drawExternalTouchMenu();
    } else if (SQUACHCAM_MENU) {
        extMenuSquachCam();
    } else {
        extMenuPig();
    }
...
void Display::pushAll() {
    ...
    // External TFT: action modes are enlarged as before. The root menu keeps
    // its purpose-built touch launcher. Every other menu/text screen is now
    // mirrored and enlarged to 320x180, with touch navigation in the bands.
    if (isBigScreenMode(porkchop.getMode())) {
        extInvalidateTouchNavOverlay();
        extPushScaledFit(topBar, mainCanvas, bottomBar);
    } else if (porkchop.getMode() == PorkchopMode::MENU && Menu::isActive()) {
        extInvalidateTouchNavOverlay();
        Menu::drawExternalTouchMenu();
    } else {
        // Mirror/enlarge text screens and keep the normal decorative bands.
        // Margin navigation buttons were removed in v22.
        extInvalidateTouchNavOverlay();
        extPushScaledFit(topBar, mainCanvas, bottomBar, true);
    }
