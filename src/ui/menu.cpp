...
bool Menu::touchWasPressed = false;
static bool externalTouchMenuDirty = true;
...
void Menu::show() {
    externalTouchMenuDirty = true;
    active = true;
    // When the menu was opened by touching the external display, do not let
    // that same held press immediately activate the button beneath it.
    touchWasPressed = extTouchPressed();
    rootIdx = 0;
    rootScroll = 0;
    activeGroup = GroupId::NONE;
    modalIdx = 0;
...
static void touchButton(int x, int y, int w, int h, const char* icon,
                        const char* label, bool selected) {
    uint16_t bg = selected ? TFT_WHITE : TFT_BLACK;
    uint16_t fg = selected ? TFT_BLACK : TFT_WHITE;
    extDisplay.fillRoundRect(x, y, w, h, 7, bg);
    extDisplay.drawRoundRect(x, y, w, h, 7, selected ? TFT_GREEN : TFT_DARKGREY);
    extDisplay.setTextDatum(MC_DATUM);
    extDisplay.setTextColor(fg, bg);
    extDisplay.setTextSize(2);
    char line[24];
    snprintf(line, sizeof(line), "%s %s", icon ? icon : "", label ? label : "");
    extDisplay.drawString(line, x + w/2, y + h/2);
}

void Menu::drawExternalTouchMenu() {
    if (!active) return;
    static bool lastReady = false;
    bool readyNow = extTouchReady();
    if (readyNow != lastReady) { externalTouchMenuDirty = true; lastReady = readyNow; }
    if (!readyNow) {
        if (extTouchCalibrationActive()) {
            extTouchCalibrationUpdate();
        } else {
            extDisplay.fillScreen(TFT_BLACK);
            extDisplay.setTextDatum(MC_DATUM);
            extDisplay.setTextColor(TFT_GREEN, TFT_BLACK);
            extDisplay.setTextSize(2);
            extDisplay.drawString("TOUCH SETUP", 160, 85);
            extDisplay.setTextSize(1);
            extDisplay.setTextColor(TFT_WHITE, TFT_BLACK);
            extDisplay.drawString("Tap the screen to calibrate", 160, 116);
            extDisplay.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
            extDisplay.drawString("Keyboard menu remains available", 160, 137);
        }
        return;
    }

    if (!externalTouchMenuDirty) return;
    externalTouchMenuDirty = false;
    extDisplay.startWrite();
    extDisplay.fillScreen(TFT_BLACK);
    extDisplay.setTextDatum(TC_DATUM);
    extDisplay.setTextColor(TFT_GREEN, TFT_BLACK);
    extDisplay.setTextSize(2);
    extDisplay.drawString(activeGroup == GroupId::NONE ? "M5PORKCHOP" : getGroupName(activeGroup), 160, 5);
    extDisplay.setTextSize(1);
    extDisplay.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    extDisplay.drawString("TOUCH A BUTTON  |  KEYBOARD STILL ACTIVE", 160, 25);

    if (activeGroup == GroupId::NONE) {
        const int bw=148, bh=54, gap=8, x0=8, y0=43;
        for (uint8_t i=0;i<ROOT_COUNT;i++) {
            int col=i%2, row=i/2;
            touchButton(x0+col*(bw+gap), y0+row*(bh+gap), bw, bh,
                        ROOT_ITEMS[i].icon, ROOT_ITEMS[i].label, i==rootIdx);
        }
    } else {
        const MenuItem* items=getGroupItems(activeGroup);
        uint8_t count=getGroupSize(activeGroup);
        const int bw=148, bh=42, gap=6, x0=8, y0=42;
        for(uint8_t i=0;i<count;i++) {
            int col=i%2, row=i/2;
            touchButton(x0+col*(bw+gap), y0+row*(bh+gap), bw, bh,
                        items[i].icon, items[i].label, i==modalIdx);
        }
        touchButton(86, 200, 148, 32, "<", "BACK", false);
    }
    extDisplay.endWrite();
}
...
void Menu::handleTouch() {
    if (!extTouchReady()) {
        if (extTouchCalibrationActive()) {
            extTouchCalibrationUpdate();
        } else if (extTouchPressed()) {
            extTouchStartCalibration();
        }
        return;
    }
    bool pressed=extTouchPressed();
    if (!pressed) { touchWasPressed=false; return; }
    if (touchWasPressed) return;
    ExtTouchPoint p;
    ...
    if (activeGroup == GroupId::NONE) {
        ...
        for(uint8_t i=0;i<ROOT_COUNT;i++) {
            int col=i%2,row=i/2;
            int x=x0+col*(bw+gap), y=y0+row*(bh+gap);
            if(p.x>=x && p.x<x+bw && p.y>=y && p.y<y+bh) {
                externalTouchMenuDirty = true;
                rootIdx=i; SFX::play(SFX::MENU_CLICK);
                const RootItem& item=ROOT_ITEMS[i];
                if(item.type==RootType::GROUP) {
                    activeGroup=item.groupId; modalIdx=0; modalScroll=0;
                } else if(item.type==RootType::DIRECT && callback) callback(item.actionId);
                drawExternalTouchMenu();
                return;
            }
        }
    } else {
        if(p.y>=198 && p.y<238 && p.x>=70 && p.x<250) {
            externalTouchMenuDirty = true;
            closeModal(); SFX::play(SFX::MENU_CLICK); drawExternalTouchMenu(); return;
        }
        const MenuItem* items=getGroupItems(activeGroup); uint8_t count=getGroupSize(activeGroup);
        const int bw=148,bh=42,gap=6,x0=8,y0=42;
        for(uint8_t i=0;i<count;i++) {
            int col=i%2,row=i/2; int x=x0+col*(bw+gap),y=y0+row*(bh+gap);
            if(p.x>=x && p.x<x+bw && p.y>=y && p.y<y+bh) {
                externalTouchMenuDirty = true;
                modalIdx=i; SFX::play(SFX::MENU_CLICK);
                if(callback) callback(items[i].actionId);
                closeModal();
                return;
            }
        }
