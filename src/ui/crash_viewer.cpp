...
void CrashViewer::update() {
    ...
    if (nukeConfirmActive) {
        if (M5Cardputer.Keyboard.isKeyPressed('y') || M5Cardputer.Keyboard.isKeyPressed('Y')) {
            ...
            scanCrashFiles();
                   ...
                   (M5Cardputer.Keyboard.isKeyPressed(KEY_BACKSPACE) || extTouchConsume(ExtTouchNav::BACK)) || (keys.enter || extTouchConsume(ExtTouchNav::SELECT))) {
            nukeConfirmActive = false;
            Display::clearBottomOverlay();
        }
        return;
    ...
    if (fileViewActive) {
        ...
        } else if ((M5Cardputer.Keyboard.isKeyPressed('.') || extTouchConsume(ExtTouchNav::DOWN))) {
            if (totalLines > VISIBLE_LINES && fileScroll < totalLines - VISIBLE_LINES) {
                fileScroll++;
            }
        } else if ((M5Cardputer.Keyboard.isKeyPressed(KEY_BACKSPACE) || extTouchConsume(ExtTouchNav::BACK)) || (keys.enter || extTouchConsume(ExtTouchNav::SELECT))) {
            fileViewActive = false;
            fileLines.clear();
            totalLines = 0;
            activeFile[0] = '\0';
    ...
    } else if ((M5Cardputer.Keyboard.isKeyPressed(KEY_BACKSPACE) || extTouchConsume(ExtTouchNav::BACK))) {
        hide();
    } else if ((keys.enter || extTouchConsume(ExtTouchNav::SELECT))) {
