...
// ---------------- External XPT2046 touch ----------------
// Separate software-SPI pins keep touch traffic away from the SD/TFT bus.
static constexpr int TOUCH_CLK  = 15;
static constexpr int TOUCH_CS   = 4;
static constexpr int TOUCH_DIN  = 13;
static constexpr int TOUCH_DO   = 8;
static constexpr int TOUCH_IRQ  = 9;
static constexpr int TOUCH_SAMPLES = 7;

// Fixed touch mapping: no calibration screen and no NVS dependency.
// These defaults cover the normal useful range of an XPT2046 panel.
// Rotation/orientation is matched to the 320x240 landscape external display.
static bool touchCalibrated = true;
static bool touchCalActive = false;
static constexpr int32_t RAW_MIN = 180;
static constexpr int32_t RAW_MAX = 3900;

static uint8_t touchShift(uint8_t out) {
    uint8_t in = 0;
    for (int b = 7; b >= 0; --b) {
        digitalWrite(TOUCH_DIN, (out >> b) & 1);
        digitalWrite(TOUCH_CLK, HIGH);
        delayMicroseconds(1);
        in = (uint8_t)((in << 1) | digitalRead(TOUCH_DO));
...
static uint16_t touchCommand(uint8_t cmd) {
    touchShift(cmd);
    uint16_t v = (uint16_t)touchShift(0) << 8;
    v |= touchShift(0);
    return (v >> 3) & 0x0FFF;
}

static bool touchRaw(int32_t& rx, int32_t& ry) {
    if (digitalRead(TOUCH_IRQ) != LOW) return false;
    uint16_t xs[TOUCH_SAMPLES], ys[TOUCH_SAMPLES];
    digitalWrite(TOUCH_CS, LOW);
    delayMicroseconds(2);
    for (int i = 0; i < TOUCH_SAMPLES; ++i) {
        xs[i] = touchCommand(0xD0);
        ys[i] = touchCommand(0x90);
    }
    digitalWrite(TOUCH_CS, HIGH);
    for (int i = 1; i < TOUCH_SAMPLES; ++i) {
        uint16_t vx = xs[i], vy = ys[i]; int j = i - 1;
        while (j >= 0 && xs[j] > vx) { xs[j + 1] = xs[j]; --j; } xs[j + 1] = vx;
        j = i - 1;
        while (j >= 0 && ys[j] > vy) { ys[j + 1] = ys[j]; --j; } ys[j + 1] = vy;
    }
    rx = xs[TOUCH_SAMPLES / 2];
    ry = ys[TOUCH_SAMPLES / 2];
    return rx > 20 && ry > 20 && rx < 4095 && ry < 4095;
}

void extTouchInit() {
    pinMode(TOUCH_CLK, OUTPUT); digitalWrite(TOUCH_CLK, LOW);
    pinMode(TOUCH_CS, OUTPUT);  digitalWrite(TOUCH_CS, HIGH);
    pinMode(TOUCH_DIN, OUTPUT); digitalWrite(TOUCH_DIN, LOW);
...
bool extTouchReady() { return true; }
bool extTouchPressed() { return digitalRead(TOUCH_IRQ) == LOW; }

bool extTouchRead(ExtTouchPoint& p) {
    int32_t rx, ry;
    if (!touchRaw(rx, ry)) return false;

    // Typical landscape XPT2046 orientation: screen X follows raw Y,
    // screen Y follows inverted raw X.
    int x = (int)((ry - RAW_MIN) * 319L / (RAW_MAX - RAW_MIN));
    int y = (int)((RAW_MAX - rx) * 239L / (RAW_MAX - RAW_MIN));
    if (x < 0) x = 0; else if (x > 319) x = 319;
    if (y < 0) y = 0; else if (y > 239) y = 239;

    // On this panel/rotation, the axis that appeared horizontally mirrored
    // is corrected by reversing the mapped Y source, not the mapped X output.
    // Keep X as mapped and reverse Y only.
    y = 239 - y;
...
// Kept for API compatibility. v15 intentionally has no calibration mode.
void extTouchStartCalibration() { touchCalActive = false; }
bool extTouchCalibrationActive() { return false; }
void extTouchForgetCalibration() { touchCalibrated = true; touchCalActive = false; }

// ---------------- Generic mirrored-screen touch navigation ----------------
static volatile ExtTouchNav g_touchNav = ExtTouchNav::NONE;
static bool navTouchWasDown = false;
static bool navOverlayDrawn = false;

void extTouchServiceNav() {
    if (!touchCalibrated || touchCalActive) return;
    bool down = extTouchPressed();
    if (!down) { navTouchWasDown = false; return; }
    if (navTouchWasDown) return;
    ExtTouchPoint p;
    if (!extTouchRead(p)) return;
    navTouchWasDown = true;

    // Top 30px: LEFT | UP | DOWN | RIGHT. Bottom 30px: BACK | SELECT.
    if (p.y < 30) {
        if      (p.x < 80)  g_touchNav = ExtTouchNav::LEFT;
        else if (p.x < 160) g_touchNav = ExtTouchNav::UP;
        else if (p.x < 240) g_touchNav = ExtTouchNav::DOWN;
        else                g_touchNav = ExtTouchNav::RIGHT;
    } else if (p.y >= 210) {
        g_touchNav = (p.x < 160) ? ExtTouchNav::BACK : ExtTouchNav::SELECT;
    }
}

bool extTouchConsume(ExtTouchNav nav) {
    if (g_touchNav != nav) return false;
    g_touchNav = ExtTouchNav::NONE;
    return true;
}

bool extTouchNavPending() {
    return g_touchNav != ExtTouchNav::NONE;
}

static void navButton(int x, int y, int w, int h, const char* label) {
    extDisplay.fillRect(x, y, w, h, TFT_BLACK);
    extDisplay.drawRect(x, y, w, h, TFT_DARKGREY);
    extDisplay.setTextDatum(MC_DATUM);
    extDisplay.setTextColor(TFT_WHITE, TFT_BLACK);
    extDisplay.setTextSize(1);
    extDisplay.drawString(label, x + w/2, y + h/2);
}

void extInvalidateTouchNavOverlay() {
    navOverlayDrawn = false;
}

void extDrawTouchNavOverlay() {
    // pushAll() may run many times per second. Repainting these opaque bands on
    // every frame caused the visible flashing and made presses hard to consume.
    if (navOverlayDrawn) return;
    navOverlayDrawn = true;
    extDisplay.startWrite();
    navButton(0,   0, 80, 30, "< LEFT");
    navButton(80,  0, 80, 30, "^ UP");
    navButton(160, 0, 80, 30, "v DOWN");
    navButton(240, 0, 80, 30, "RIGHT >");
    navButton(0, 210, 160, 30, "< BACK");
    navButton(160,210, 160, 30, "OK / SELECT");
    extDisplay.endWrite();
}
static void touchDrawTarget(int x,int y){
    extDisplay.drawCircle(x,y,12,TFT_WHITE); extDisplay.drawCircle(x,y,4,TFT_GREEN);
    extDisplay.drawFastHLine(x-18,y,36,TFT_WHITE); extDisplay.drawFastVLine(x,y-18,36,TFT_WHITE);
}
void extTouchCalibrationUpdate() {}
...
// Proven low-level ILI9341 wake/init sequence. This runs before M5GFX takes
// ownership of the bus. It intentionally does not touch NVS, SPIFFS, stats,
// or any persistent partition.
static void extProvenHardwareInit() {
    static constexpr int TFT_CS   = 5;
    static constexpr int TFT_RST  = 3;
    static constexpr int TFT_DC   = 6;
    static constexpr int TFT_MOSI = 14;
    static constexpr int TFT_SCLK = 40;

    SPIClass bootstrapSpi(FSPI);
    ...
    bootstrapSpi.begin(TFT_SCLK, -1, TFT_MOSI, TFT_CS);
    bootstrapSpi.beginTransaction(SPISettings(20000000, MSBFIRST, SPI_MODE0));

    auto command = [&](uint8_t value) {
        digitalWrite(TFT_DC, LOW);
        digitalWrite(TFT_CS, LOW);
        bootstrapSpi.transfer(value);
        digitalWrite(TFT_CS, HIGH);
    };
    auto data8 = [&](uint8_t value) {
        digitalWrite(TFT_DC, HIGH);
        digitalWrite(TFT_CS, LOW);
        bootstrapSpi.transfer(value);
        digitalWrite(TFT_CS, HIGH);
    };
    auto data = [&](const uint8_t* values, size_t count) {
        digitalWrite(TFT_DC, HIGH);
        digitalWrite(TFT_CS, LOW);
        bootstrapSpi.writeBytes(values, count);
        digitalWrite(TFT_CS, HIGH);
    };

    // Hardware reset, exactly as used by the standalone color test.
    digitalWrite(TFT_RST, HIGH); delay(10);
    digitalWrite(TFT_RST, LOW);  delay(20);
    digitalWrite(TFT_RST, HIGH); delay(150);

    command(0x01); delay(150); // SWRESET
    command(0x28);             // DISPOFF

    { const uint8_t v[] = {0x00,0xC1,0x30}; command(0xCF); data(v,sizeof(v)); }
    { const uint8_t v[] = {0x64,0x03,0x12,0x81}; command(0xED); data(v,sizeof(v)); }
    { const uint8_t v[] = {0x85,0x00,0x78}; command(0xE8); data(v,sizeof(v)); }
    { const uint8_t v[] = {0x39,0x2C,0x00,0x34,0x02}; command(0xCB); data(v,sizeof(v)); }
    command(0xF7); data8(0x20);
    { const uint8_t v[] = {0x00,0x00}; command(0xEA); data(v,sizeof(v)); }
    command(0xC0); data8(0x23);
    command(0xC1); data8(0x10);
    { const uint8_t v[] = {0x3E,0x28}; command(0xC5); data(v,sizeof(v)); }
    command(0xC7); data8(0x86);
    command(0x36); data8(0x28); // landscape
    command(0x3A); data8(0x55); // RGB565
    { const uint8_t v[] = {0x00,0x18}; command(0xB1); data(v,sizeof(v)); }
    { const uint8_t v[] = {0x08,0x82,0x27}; command(0xB6); data(v,sizeof(v)); }
    command(0xF2); data8(0x00);
    command(0x26); data8(0x01);
    { const uint8_t v[] = {0x0F,0x31,0x2B,0x0C,0x0E,0x08,0x4E,0xF1,0x37,0x07,0x10,0x03,0x0E,0x09,0x00}; command(0xE0); data(v,sizeof(v)); }
    { const uint8_t v[] = {0x00,0x0E,0x14,0x03,0x11,0x07,0x31,0xC1,0x48,0x08,0x0F,0x0C,0x31,0x36,0x0F}; command(0xE1); data(v,sizeof(v)); }
    command(0x11); delay(150); // SLPOUT
    command(0x29); delay(50);  // DISPON

    bootstrapSpi.endTransaction();
    bootstrapSpi.end();
    delay(20);
}
...
void extPushScaledFit(M5Canvas& top, M5Canvas& main, M5Canvas& bottom, bool animateBands) {
    ...
    if (animateBands) {
        // Animated/action screens own the bands, so the cached navigation
        // overlay must be redrawn next time a text screen is entered.
        extInvalidateTouchNavOverlay();
        extDrawBands();
    }
    // When animateBands is false, deliberately leave the 30px bands alone.
    // extDrawTouchNavOverlay() paints them once after the scaled center content
    // is updated; subsequent frames update only the 320x180 center region.
}
