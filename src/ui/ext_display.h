...
// Touch (isolated software SPI; GPS/LoRa must remain off):
//   T_CLK=GPIO15 T_CS=GPIO4 T_DIN=GPIO13 T_DO=GPIO8 T_IRQ=GPIO9
...
// External resistive touch (XPT2046-compatible controller). The touch bus is
// intentionally bit-banged on otherwise-unused EXT pins so it cannot contend
// with the SD card / external TFT hardware SPI bus.
struct ExtTouchPoint {
    int16_t x;
    int16_t y;
};
void extTouchInit();
bool extTouchReady();             // true after saved calibration exists
bool extTouchPressed();
bool extTouchRead(ExtTouchPoint& p); // calibrated 320x240 coordinates
void extTouchStartCalibration();
bool extTouchCalibrationActive();
void extTouchCalibrationUpdate(); // draws/advances the calibration screen
void extTouchForgetCalibration();

// Generic touch navigation used by mirrored menu/text screens.
enum class ExtTouchNav : uint8_t { NONE=0, LEFT, UP, DOWN, RIGHT, BACK, SELECT };
void extTouchServiceNav();              // poll once per main loop
bool extTouchConsume(ExtTouchNav nav);  // one-shot event for the active screen
bool extTouchNavPending();              // true when a band button is waiting to be consumed
void extDrawTouchNavOverlay();          // controls in the 30px letterbox bands (cached)
void extInvalidateTouchNavOverlay();      // force controls to redraw after another screen owns the bands
...
void extPushScaledFit(M5Canvas& top, M5Canvas& main, M5Canvas& bottom, bool animateBands = true);
