...
void UnlockablesMenu::handleInput() {
    ...
    if ((keys.enter || extTouchConsume(ExtTouchNav::SELECT)) && TOTAL_UNLOCKABLES > 0 && selectedIndex < TOTAL_UNLOCKABLES) {
...
void UnlockablesMenu::handleTextInput() {
    ...
    bool hasActionKey = (keys.enter || extTouchConsume(ExtTouchNav::SELECT)) || keys.del;
